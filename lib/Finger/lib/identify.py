...
import re
import json
from lib.Finger.config.data import path
from lib.Finger.config.color import color
from urllib.parse import urlsplit
from lib.Finger.config.data import logging, Webinfo


class Identify:
    def __init__(self):
        filepath = "lib/Finger/library/finger.json"
    ...
    def run(self, datas):
        ...
        if _url in _webinfo and self.datas["title"] in _webinfo:
            ...
            Msg = "{0} {1} {4} {3} {2}".format(color.green(self.datas['cms']),
                                               color.blue(self.datas['Server']), self.datas['title'][:40],
