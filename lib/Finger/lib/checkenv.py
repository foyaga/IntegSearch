...
import requests
import hashlib
from lib.Finger.config.config import head,FingerPrint_Update
from lib.Finger.config.data import path,logging
...
class CheckEnv:
    ...
    def update(self):
        try:
            ...
            response = requests.get(Fingerprint_Page,timeout = 5,headers = head)
